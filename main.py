...
def multiModel(SLICES_COLLECT, dataset):
    
    models = []
    for SLICE in SLICES_COLLECT:
        models.append(U_Net(in_channels=SLICE, out_channels = 32, dataset=dataset)) #out_channels = 32
...
def train3D(dataset):
    model = UNet(in_dim=1, out_dim=2, num_filters=32, dataset=dataset)
    if device.type == "cuda":
        print("GPU: ", torch.cuda.device_count())
...
if __name__ == "__main__":
    ...
    save_file_name = DATASET + "{0}/history{1}/history_Valid.csv".format(fileName, modelName)
    save_dir = DATASET + "{0}/history{1}/".format(fileName, modelName)

    # Saving images and models directories
    model_save_dir = DATASET + "{0}/history{1}/saved_models3".format(fileName, modelName)
    image_save_path = DATASET + "{0}/history{1}/result_images3".format(fileName, modelName)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    dataset_path = 'train_' + DATASET 
    # dataset_cache = 'data_cache/{0}_dataset_cache'.format(DATASET)
    dataset_cache = 'data_cache/isbi13_fix_dataset_cache_5val'
    ...
    train_load = torch.utils.data.DataLoader(dataset=trainDataset, num_workers=6, batch_size=4, shuffle=True)
    val_load = torch.utils.data.DataLoader(dataset=validDataset, num_workers=6, batch_size=1, shuffle=False)

    # train3D(DATASET)
    if COMPLETE_TRAIN:
        models = multiModel(SLICES_COLLECT, DATASET)
        train()
